-- DOMARO V30.4 — Smart Delivery Zones
-- Run after V30.3. Safe additive migration.
...
create table if not exists public.shipping_zones (
  ...
  check (area is null or btrim(area) <> '')
);

create unique index if not exists shipping_zones_location_uidx
on public.shipping_zones (lower(btrim(governorate)), lower(btrim(coalesce(area,''))));

create index if not exists shipping_zones_governorate_idx
on public.shipping_zones (lower(btrim(governorate)))
where active = true;

alter table public.shipping_zones enable row level security;
drop policy if exists "Owner can manage shipping zones" on public.shipping_zones;
create policy "Owner can manage shipping zones"
...
-- Seed all 27 governorates using the current V30.3 fallback fee.
with govs(governorate) as (
  values
    ('Cairo'),('Giza'),('Alexandria'),('Qalyubia'),('Sharqia'),('Dakahlia'),('Gharbia'),('Monufia'),('Beheira'),
    ('Kafr El Sheikh'),('Damietta'),('Port Said'),('Ismailia'),('Suez'),('Fayoum'),('Beni Suef'),('Minya'),('Assiut'),
    ('Sohag'),('Qena'),('Luxor'),('Aswan'),('Red Sea'),('New Valley'),('Matrouh'),('North Sinai'),('South Sinai')
), cfg as (
  select coalesce((select shipping_fee from public.store_settings where id=1),80)::numeric(12,2) as fee
)
insert into public.shipping_zones(governorate,area,fee,active)
select g.governorate,null,cfg.fee,true
from govs g cross join cfg
where not exists (
  select 1 from public.shipping_zones z
  where lower(btrim(z.governorate))=lower(btrim(g.governorate))
    and nullif(btrim(coalesce(z.area,'')),'') is null
);
...
begin
  if btrim(coalesce(p_governorate,''))='' then
    raise exception 'Governorate is required' using errcode='22023';
  end if;

  select z.fee,z.active
  into v_default_fee,v_default_active
  from public.shipping_zones z
  where lower(btrim(z.governorate))=lower(btrim(p_governorate))
    and nullif(btrim(coalesce(z.area,'')),'') is null
  limit 1;
  v_has_default:=found;

  if v_has_default and not v_default_active then
    raise exception 'Delivery is currently unavailable to this governorate' using errcode='22023';
  end if;

  if btrim(coalesce(p_area,''))<>'' then
    select z.fee,btrim(z.area)
    into v_fee,v_area
    from public.shipping_zones z
    where z.active=true
      and lower(btrim(z.governorate))=lower(btrim(p_governorate))
      and nullif(btrim(coalesce(z.area,'')),'') is not null
      and lower(regexp_replace(btrim(z.area),'\s+',' ','g'))
          = lower(regexp_replace(btrim(p_area),'\s+',' ','g'))
    limit 1;
  ...
  select coalesce(shipping_fee,80) into v_fee from public.store_settings where id=1;
  if not found then v_fee:=80; end if;
  return jsonb_build_object(
    'fee',v_fee,
    'source','fallback',
    'governorate',btrim(p_governorate),
...
$$;
revoke all on function public.get_delivery_quote(text,text) from public;
...
as $$
  ...
  from (
    ...
    where active=true
      and lower(btrim(governorate))=lower(btrim(p_governorate))
...
-- New orders always calculate delivery server-side from the selected location.
...
-- Add delivery-zone changes to the existing owner audit log.
drop trigger if exists trg_v30_audit_shipping_zones on public.shipping_zones;
create trigger trg_v30_audit_shipping_zones
after insert or update or delete on public.shipping_zones
for each row execute function public.v30_audit_change();

-- Include delivery zones in owner backups.
...
begin
  ...
  return jsonb_build_object(
    ...
    'shipping_zones',(select coalesce(jsonb_agg(to_jsonb(z) order by z.governorate,z.area nulls first),'[]'::jsonb) from public.shipping_zones z)
  );
end;$$;
...
-- Extend restore so a V30 backup can also merge delivery zones when present.
...
begin
  ...
  update public.product_variants v set is_default=false,updated_at=now()
  ...
  if p_backup ? 'shipping_zones' and jsonb_typeof(p_backup->'shipping_zones')='array' then
    for x in select value from jsonb_array_elements(p_backup->'shipping_zones') loop
      update public.shipping_zones
      set fee=(x->>'fee')::numeric,active=coalesce((x->>'active')::boolean,true),updated_at=now()
      where lower(btrim(governorate))=lower(btrim(x->>'governorate'))
        and lower(btrim(coalesce(area,'')))=lower(btrim(coalesce(x->>'area','')));
      if not found then
        insert into public.shipping_zones(governorate,area,fee,active)
        values(btrim(x->>'governorate'),nullif(btrim(coalesce(x->>'area','')),''),(x->>'fee')::numeric,coalesce((x->>'active')::boolean,true));
      end if;
    end loop;
  end if;
